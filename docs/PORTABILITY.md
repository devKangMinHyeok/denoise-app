...
- 프로필·작업 기록: `~/.vocast/{profiles,history,denoise}/`
...
```bash
bash packaging/scripts/verify_cleanroom.sh   # 언제든 재실행 가능
```
...
- **mlx-whisper가 bare `ffmpeg`를 PATH에서 호출** — 오디오 로드 시. 동봉
  ...
  동봉본을 `ffmpeg` 심링크(`~/.vocast/bin`)로 만들어 PATH에 얹어 해결.
  제3자 라이브러리의 bare 호출까지 커버.

## 완전 봉인 번들 (판매용 — uv도 필요 없음)

`scripts/build_bundle.sh`가 **uv·파이썬·ffmpeg가 전혀 없는 Mac에서도 그대로
도는** self-contained 배포본 `dist/Vocast/`를 만든다.

```bash
bash packaging/scripts/build_bundle.sh        # → dist/Vocast/ (약 3.7GB)
# 사용자: 'Vocast 실행.command' 더블클릭
```
...
```bash
bash packaging/scripts/build_bundle.sh --with-models   # → 약 11GB, 네트워크 불필요
```
