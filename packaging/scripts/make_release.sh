...
#
# 사용: bash packaging/scripts/make_release.sh [--with-models] [VERSION]
#
...
set -euo pipefail
# packaging/scripts/ 에 산다 → 저장소 루트는 두 단계 위, 파이썬 엔진은 app/
REPO="$(cd "$(dirname "$0")/../.." && pwd)"
APP="$REPO/app"
WITH_MODELS=""
[ "${1:-}" = "--with-models" ] && { WITH_MODELS="--with-models"; shift; }
VERSION="${1:-$(grep -m1 '^version' "$APP/pyproject.toml" | cut -d'"' -f2)}"
OUT="$REPO/dist/release"
NAME="Vocast-macos-arm64.tar.gz"

echo "▸ 번들 빌드 ${WITH_MODELS:-(모델 미포함)}"
bash "$REPO/packaging/scripts/build_bundle.sh" $WITH_MODELS

echo "▸ 압축: $NAME"
mkdir -p "$OUT"
tar -C "$REPO/dist" -czf "$OUT/$NAME" Vocast
...
echo "  NC_URL=file://$OUT/$NAME NC_SHA256=$SHA bash packaging/scripts/install.sh"
