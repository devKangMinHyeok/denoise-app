...
> 이 저장소는 모노레포다. 파이썬 엔진(core·api·voice)은 **`app/`** 아래 있고,
> 아래 CLI·서버 명령은 전부 `app/` 에서 실행한다 (`cd app`).
...
**규칙** (app/tests/test_architecture.py 가 CI에서 강제):
...
app/                    ★ 제품 (파이썬 엔진 + 네이티브 UI)
  ...
  cli/                    denoise.py / evaluate.py (노이즈 제거·채점 CLI)
  voice/                  clone_say.py / evaluate_tts.py (클로닝 CLI)
