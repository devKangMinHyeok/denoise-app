...
Vocast 랜딩(영어, Next.js 정적 export, GitHub Pages `/vocast/`)의 Technical + On-Page SEO와
구조화 데이터를 감사한다. 한국(Naver) 항목은 영어 사이트라 제외한다.

**핵심 질문**: "검색엔진이 이 페이지를 정확히 크롤링/이해/색인할 기술 기반을 갖췄는가?"

## 파일 필터
- `landing/app/**/page.tsx`, `landing/app/layout.tsx`
- `landing/app/sitemap.ts`, `landing/app/robots.ts`
- `landing/lib/site.ts`, `landing/lib/schema.ts`

## 체크리스트

### A. 렌더링/색인
- [ ] 모든 공개 라우트가 정적 export로 프리렌더되는가? (JS 없이 콘텐츠 보임)
- [ ] 새 라우트가 `app/sitemap.ts`에 포함되는가? 절대 URL + `lastModified` 포함?
- [ ] `robots.ts`가 전체 허용 + `Sitemap:` 지시 + AI 크롤러 허용을 갖는가?
      (주의: Pages 프로젝트 경로라 robots는 `/vocast/robots.txt`. 사이트맵은 Search Console 직접 제출.)

### B. Canonical / 메타
- [ ] 모든 페이지에 self-referencing 절대 canonical이 있는가? (`abs()` 사용, `/vocast` 유지)
