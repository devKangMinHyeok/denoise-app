...
실행: python mcp_server.py   (stdio JSON-RPC)
에이전트 등록(설정 출력): python mcp_server.py --config
"""
import os
import sys
import tempfile
...
def denoise(input_path: str, output_path: str = "", mode: str = "standard",
    ...
    if mode == "resynth" and not resynth_available():
        raise ValueError("재합성 엔진 미설치: bash scripts/install_resynth.sh")
...
def list_voice_profiles() -> list:
    ...
    from web.profiles import list_profiles
...
def clone_voice(text: str, profile_id: str = "", ref_path: str = "",
    ...
    if profile_id:
        from web.profiles import profile_paths
...
def list_history(limit: int = 10) -> list:
    ...
    from web.profiles import list_history as _hist
