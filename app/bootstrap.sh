...
bash ../packaging/scripts/download_dnsmos.sh 2>/dev/null || echo "  (건너뜀 — 평가 기능 쓸 때 ../packaging/scripts/download_dnsmos.sh)"

echo
echo "선택 엔진 (원할 때 별도 설치):"
echo "  • 하이브리드 노이즈 제거(DFN):   bash ../packaging/scripts/install_dfn.sh"
echo "  • 재합성(발화 중 노이즈까지):    bash ../packaging/scripts/install_resynth.sh"
echo
echo "✅ 준비 완료. 실행:  uv run python api/server.py"
