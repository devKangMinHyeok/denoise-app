...
- dfn (하이브리드, 설치 시 기본): DeepFilterNet3+PF 2패스 —
  ...
  설치: bash packaging/scripts/install_dfn.sh (전용 py3.11 venv — 상세는 스크립트 참고)
"""
import os
import subprocess
import sys

from voxa import ROOT, RNNOISE_MODEL
from voxa.media.audio import audio_codec_args, has_video_stream, run_ffmpeg
...
def _denoise_wav_dfn(in_wav, out_wav, on_progress=None):
    ...
    import tempfile
    worker = os.path.join(ROOT, "denoise", "dfn_worker.py")
    with tempfile.TemporaryDirectory() as wd:
...
def _resynth_wav(in_wav, out_wav, on_progress=None):
    ...
    worker = os.path.join(ROOT, "denoise", "resynth_worker.py")
...
def run_denoise(input_path, output_path, boost=0.0, engine="auto",
    ...
    if use_dfn and not dfn_available():
        raise RuntimeError("DFN 엔진이 설치되어 있지 않습니다: bash packaging/scripts/install_dfn.sh")
